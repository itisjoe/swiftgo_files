...
class ViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        myDatePicker.addTarget(self, action: #selector(ViewController.datePickerChanged), forControlEvents: .ValueChanged)
        
        // 將 UITextField 原先鍵盤的視圖更換成 UIDatePicker
        myTextField.inputView = myDatePicker

        // 設置 UITextField 預設的內容
        myTextField.text = formatter.stringFromDate(myDatePicker.date)
    ...
    func datePickerChanged(datePicker:UIDatePicker) {
        ...
        myTextField?.text = formatter.stringFromDate(datePicker.date)
    }
